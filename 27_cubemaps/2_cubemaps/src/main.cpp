 ...
 * Breif Description : basic cube maps; Note face culling is only done for skybox since the winding order for the box is not proper. If it was then we could have globally enabled face culling
 * Detailed Description : unable to figure out how to add depth test to this (dont even know if I am onto something there). The cubemap has to move with the camera - not going to implment this because its super easy. 
 *****************************************************************************/
#define GLEW_STATIC
#include <GL/glew.h>
...
int main()
{
	...
	while(!glfwWindowShouldClose(window))
	{
		...
		glm::mat4 view = glm::lookAt(glm::vec3(sin(counter)*5, 0.2f, cos(counter)*5), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		//glm::mat4 view = glm::lookAt(glm::vec3(0.0f, 0.2f, 5.0f), glm::vec3(0.0f, 0.0f, 0.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		...
		glBindTexture(GL_TEXTURE_2D, texture_box);
		glUniform1i(glGetUniformLocation(our_shader.program, "mytexture"), 1);
		...
		glUniform1i(glGetUniformLocation(skybox_shader.program, "skybox"), 2);
