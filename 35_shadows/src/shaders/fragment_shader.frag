...
void main()
{
	...
	float specular_value = specular_strength * pow(max(-dot(view_vector, reflected_vector), 0.0), 1); // the shininess property is made very poor
	vec3 specular_light = specular_value * light_color;  
		
	//color = vec4(ambient_light + diffuse_light, 1.0f) * vec4(floor_color, 1.0f);
	color = vec4(ambient_light + diffuse_light + specular_light, 1.0f) * texture(mytexture, TexCoord); 
} 
