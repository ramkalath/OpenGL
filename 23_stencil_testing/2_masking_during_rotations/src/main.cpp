 ...
 * Detailed Description : there is a redish rectangle behind and a triangular stencil. I pretty much want only the area within the triangle to emit the contents behind. The area outside the triangle is masked. Its like looking through onto a red wall paper with a triangulare hole cut on a piece of paper. The stencil buffer bit is cleared at every iteration.
 *****************************************************************************/
...
int main()
{
	...
	GLfloat angle = 45.0f, n = 0.2f, f = 20.0f, ar = (GLfloat)width/(GLfloat)height;
	...
	while(!glfwWindowShouldClose(window))
	{
		...
		glBindVertexArray(VAO_tri);
		glUniformMatrix4fv(glGetUniformLocation(simple_shader.program, "model"), 1, GL_FALSE, glm::value_ptr(model1));
		glUniformMatrix4fv(glGetUniformLocation(simple_shader.program, "view"), 1, GL_FALSE, glm::value_ptr(view));
		glUniformMatrix4fv(glGetUniformLocation(simple_shader.program, "projection"), 1, GL_FALSE, glm::value_ptr(projection));
		...
		glBindVertexArray(VAO_rect);
		glUniformMatrix4fv(glGetUniformLocation(simple_shader.program, "model"), 1, GL_FALSE, glm::value_ptr(model2));
		glUniformMatrix4fv(glGetUniformLocation(simple_shader.program, "view"), 1, GL_FALSE, glm::value_ptr(view));
		glUniformMatrix4fv(glGetUniformLocation(simple_shader.program, "projection"), 1, GL_FALSE, glm::value_ptr(projection));
