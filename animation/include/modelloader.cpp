		...
		for(unsigned int j=0; j<node->mNumMeshes; j++)
		{
			...
			if(mesh->HasBones())
			{
				for(int i=0; i<mesh->mNumBones; i++)
				{
					...
					{
						std::cout << bone->mWeights[j].mWeight << std::endl; // got each bone's associated vertex's weight;
						// TODO(ram): dunno yet how weights map to vertices
						//std::cout << bone->mWeights->mVertexId << std::endl;
					}
				}
			}
