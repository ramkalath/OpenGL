...
int main()
{
	...
	Shader cube_shader("./shaders/object_vertex_shader.vert", "./shaders/object_fragment_shader.frag");

	// ================================================================================
	// Loading models using custom model loaders
	GameSettings globalsettings;
	Modelloader cube("./resources/multi_res_cube/multires_cube.obj");
	...
	while(!glfwWindowShouldClose(window))
	{
		...
		glUseProgram(cube_shader.program);
		 ...
		 // Uniforms
		glUniformMatrix4fv(glGetUniformLocation(cube_shader.program, "view"), 1, GL_FALSE, glm::value_ptr(globalsettings.view));
		glUniformMatrix4fv(glGetUniformLocation(cube_shader.program, "projection"), 1, GL_FALSE, glm::value_ptr(globalsettings.projection_perspective));
		glUniform3f(glGetUniformLocation(cube_shader.program, "LightDirection"), 0.0f, 10.0f, 0.0f);
		glUniform3f(glGetUniformLocation(cube_shader.program, "LightAmbient"), 0.5f, 0.5f, 0.5f);
		glUniform3f(glGetUniformLocation(cube_shader.program, "LightDiffuse"), 1.0f, 1.0f, 1.0f);
		glUniform3f(glGetUniformLocation(cube_shader.program, "LightSpecular"), 1.0f, 1.0f, 1.0f);
		glUniform3f(glGetUniformLocation(cube_shader.program, "CameraPosition"), view_pos.x, view_pos.y, view_pos.z);

		// render nanosuit
		//glUniformMatrix4fv(glGetUniformLocation(cube_shader.program, "model"), 1, GL_FALSE, glm::value_ptr(cube.modelmatrix));
